...
# Path to uploaded project PDF (provided in your environment)
...
def show_model_missing_message(model_path: str, model_type: str = "model"):
    ...
    st.info("Make sure you trained the model and saved it at this path, or update the app settings to point to the correct file.")
    st.caption("You can press the red stop button and fix the path, then rerun the app.")
...
def add_pdf_download_button():
    ...
    if p.exists():
        with open(p, "rb") as f:
            data = f.read()
        st.download_button(label="Download project PDF", data=data, file_name=p.name, mime="application/pdf")
    else:
