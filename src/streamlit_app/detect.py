...
from .utils import is_image_file, read_image_from_upload, show_model_missing_message, display_image

# default YOLO path (user can change)
...
def run_yolo_on_image(model, pil_image: Image.Image, imgsz: int = 640, conf: float = 0.25, iou: float = 0.45, save_dir: str = "outputs/yolo_infer", save_name: str = "streamlit"):
    """
    Runs model.predict on a single PIL image, returns results object and path to annotated image (if saved).
    """
    # save PIL to temp file then call model.predict with path
    tmp_dir = Path(tempfile.mkdtemp())
    tmp_img_path = tmp_dir / "input.jpg"
    pil_image.save(tmp_img_path, format="JPEG")
    try:
        results = model.predict(source=str(tmp_img_path), imgsz=imgsz, conf=conf, iou=iou, save=True,
                                project=save_dir, name=save_name, exist_ok=True)
        # ultralytics writes outputs in save_dir/name
        out_dir = Path(save_dir) / save_name
        # pick annotated file if exists
        annotated = None
        for ext in [".jpg", ".png"]:
...
def detection_page():
    ...
    st.write("Upload an image for YOLOv8 detection (or use pretrained yolov8n).")
    ...
    if st.button("Load YOLO model"):
        ...
        if model is None:
            show_model_missing_message(weights_path, model_type="YOLO model")
        else:
    ...
    if uploaded:
        ...
        # Quick text summary
        try:
            # results is a list of Results — show first
            r = results[0]
            boxes = r.boxes
